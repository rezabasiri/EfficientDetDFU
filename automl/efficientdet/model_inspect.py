...
class ModelInspector(object):
  ...
  def saved_model_inference(self, image_path_pattern, output_dir, **kwargs):
    ...
    for i in range(num_batches):
      ...
      image_filename = os.path.basename(' '.join(map(str, batch_files)))
      ...
      for j in range(size_before_pad):
        #print('detections_bs=',detections_bs[j])
        (img, datatemp) = driver.visualize(raw_images[j], detections_bs[j], **kwargs)
        #print('bbox coordinates', datatemp)
        if datatemp is not None:
          print('bbox coordinates', len(datatemp))
          for m in range(len(datatemp)):
            #print('len',datatemp[m])
            to_export_csv = [image_filename] + list(datatemp[m])
            print('to_export_csv',to_export_csv)
            df = pd.DataFrame([to_export_csv], columns=['filename', 'xmin', 'ymin', 'xmax', 'ymax', 'score'])
            self.datacsv = self.datacsv.append(df)
            #print('test',df)
        img_id = str(i * batch_size + j)
        output_image_path = os.path.join(output_dir, image_filename)
        Image.fromarray(img).save(output_image_path)
        logging.info('writing file to %s', output_image_path)
    self.datacsv.to_csv('PredictedTruth.csv', index=False, header=True)
