...
def visualize_boxes_and_labels_on_image_array(
  ...
  for i in range(boxes.shape[0]):
    ...
    if scores is None or scores[i] > min_score_thresh:
      #print('scoreInVis_Util:',scores[i])
      box = tuple(boxes[i].tolist())
      ymin, xmin, ymax, xmax = box
      print('box',box)
      ttt = (int(xmin), int(ymin), int(xmax), int(ymax), float(scores[i]))
      datatemp.append(ttt)
      print('datatempInVis_Util=',datatemp)
      if instance_masks is not None:
  ...
  for box, color in box_to_color_map.items():
    ymin, xmin, ymax, xmax = box
    #print('box2inVis_Util',ymin, xmin, ymax, xmax)
    #ttest.append(box)
    #print('ttest',ttest)
    ...
    #return box
  print('datatempFinalVis_Utils', datatemp)
  return (image, datatemp)
